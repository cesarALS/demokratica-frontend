...
const Header = () => {
  return (
    ...
    <nav
      className={`bg-SecBlue flex items-center justify-center w-full h-full`}
    >
      <div className="flex justify-center items-center w-[15%] h-full">
        <div className="flex items-center justify-center w-[80%] h-[90%]">
          <Link href={"/"} passHref title="Demokratica">
            <TitleLogo
              baseFill="#000000"
              classNameGeneral={`w-[80%] ${hoverScale(110, 100)}`}
            />
          </Link>
        </div>
      </div>
      ...
      <div className="flex justify-center items-center w-[15%] h-full">
        <Link
          ...
          passHref
          title="Loguearse"
          className={`flex items-center justify-center bg-PrimCreamCan rounded-md px-8 border-[0.15em] border-black h-[50%] w-[60%] ${hoverScale(
            115,
            100
          )}`}
        >
...
const HeaderLink = ({ name, link }: HeaderLinkProps) => {
  return (
    <Link
      href={link}
      className={`object-cover text-lg ${hoverScale(115, 100)}`}
    >
      {name}
    </Link>
  );
